...
const P = {
  ID: 0, POST_DATE: 2, POST_DATE_GMT: 3, POST_CONTENT: 4,
  POST_TITLE: 5, POST_EXCERPT: 6, POST_STATUS: 7, POST_NAME: 11,
  POST_MODIFIED: 14, POST_MODIFIED_GMT: 15, POST_PARENT: 17,
  GUID: 18, POST_TYPE: 20, POST_MIME_TYPE: 21,
} as const

// gi_comments column indices
const C = {
  ID: 0, POST_ID: 1, AUTHOR: 2, EMAIL: 3, DATE: 6, DATE_GMT: 7,
  CONTENT: 8, APPROVED: 10, TYPE: 12, PARENT: 13,
} as const
...
async function migrate() {
  ...
  for (const row of relevantPosts) {
    ...
    const content = htmlToMarkdown(row[P.POST_CONTENT] as string)
    ...
    const updatedAt = parseWpDate(row[P.POST_MODIFIED_GMT] as string, row[P.POST_MODIFIED] as string)
    ...
    const result = await client.query(
      `INSERT INTO posts (title, slug, content, excerpt, featured_image, post_type, status, published_at, created_at, updated_at, wp_original_id)
       ...
       RETURNING id`,
      [row[P.POST_TITLE], slug, content, excerpt, featuredImage, postType, status, publishedAt, createdAt, updatedAt, wpId],
    )
  ...
  const giSigs = new Set(
    approvedGi.map((r) => `${r[C.POST_ID]}-${r[C.EMAIL]}-${r[C.DATE_GMT]}`),
  )
  ...
  for (const comment of allComments) {
    ...
    if (childUuid && parentUuid) {
      await client.query(`UPDATE comments SET parent_id = $1 WHERE id = $2`, [parentUuid, childUuid])
      threadedCount++
    }
  }
  ...
  const termsByTaxonomy = { post_tag: new Set<number>(), sessao: new Set<number>(), cursos: new Set<number>() }
  ...
  for (const row of relationshipRows) {
    ...
    if (tagUuid) {
      await client.query(`INSERT INTO post_tags (post_id, tag_id) VALUES ($1, $2) ON CONFLICT DO NOTHING`, [postUuid, tagUuid])
      relCounts.tags++
      continue
    }
    ...
    if (sectionUuid) {
      await client.query(`INSERT INTO post_sections (post_id, section_id) VALUES ($1, $2) ON CONFLICT DO NOTHING`, [postUuid, sectionUuid])
      relCounts.sections++
      continue
    }
    ...
    if (courseUuid) {
      await client.query(`INSERT INTO post_courses (post_id, course_id) VALUES ($1, $2) ON CONFLICT DO NOTHING`, [postUuid, courseUuid])
      relCounts.courses++
      continue
    }
  }
  ...
  console.log('\n=== Migration Summary ===')
  console.log(`Posts: ${s.posts} | Comments: ${s.comments} | Tags: ${s.tags} | Sections: ${s.sections} | Courses: ${s.courses}`)
